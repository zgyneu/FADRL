...
class PPO:
    def __init__(self, problem_name, size, opts):
        ...
        self.actor = Actor(
            ...
            seq_length = size + opts.num_couriers
        )
    ...
    def rollout(self, problem, val_m, batch, do_sample = False, show_bar = False, num_couriers=1):
        ...
        solutions = move_to(problem.get_initial_solutions(batch, val_m, num_couriers), self.opts.device).long()
...
def train(rank, problem, agent, val_dataset, tb_logger):
    ...
    for epoch in range(opts.epoch_start, opts.epoch_end):
        ...
        training_dataset = problem.make_dataset(size=opts.graph_size, num_samples=opts.epoch_size, num_couriers=opts.num_couriers)
        if opts.distributed:
...
def train_batch(
    ...
    solution = move_to_cuda(problem.get_initial_solutions(batch),rank) if opts.distributed \
                        else move_to(problem.get_initial_solutions(batch, num_couriers = opts.num_couriers), opts.device)
    obj = problem.get_costs(batch, solution, num_couriers = opts.num_couriers)
