...
class CompatNeighbour(nn.Module):
    ...
    def forward(self, h, rec, visited_order_map, selection_sig, num_couriers = 1):
        ...
        compatibility = ((Q_pre * hidden_K).sum(-1) + (hidden_Q * K_post).sum(-1) - (Q_pre * K_post).sum(-1))[:,:,num_couriers:]
        
        compatibility_pairing = torch.cat((compatibility[:,:,:(graph_size-num_couriers) // 2], compatibility[:,:,(graph_size-num_couriers) // 2:]), 0)
...
class MultiHeadDecoder(nn.Module):
    ...
    def forward(self, problem, h_em, rec, x_in, top2, visited_order_map, pre_action, selection_sig, fixed_action = None, require_entropy = False):        
        ...
        bs, gs, dim = h_em.size()
        half_pos =  (gs - problem.num_couriers) // 2
        ...
        ############# action1 removal
        if TYPE_REMOVAL == 'N2S':
            action_removal_table = torch.tanh(self.compater_removal(h, rec, visited_order_map, selection_sig, problem.num_couriers).squeeze()) * self.range
        ...
        ############# action2
        pos_pickup = (problem.num_couriers + action_removal).view(-1)
        pos_delivery = pos_pickup + half_pos
        mask_table = problem.get_swap_mask(action_removal + problem.num_couriers, visited_order_map, top2, rec, problem.num_couriers).expand(bs, gs, gs).cpu()
        if TYPE_REINSERTION == 'N2S':
        ...
        if fixed_action is not None:
            ...
            if TYPE_REINSERTION == 'greedy':
                ...
                # sample one action
                dist=Categorical(probs=probs_reinsertion)
                pair_index=dist.sample().unsqueeze(-1)
                while (probs_reinsertion[torch.arange(bs),pair_index.squeeze(-1)]==0).any():
                    pair_index=dist.sample().unsqueeze(-1)
...
class EmbeddingNet(nn.Module):
    ...
    def position_encoding(self, solutions, embedding_dim, clac_stacks = False, problem=None):
         ...
         _, visited_time = problem.getMcSeqBatch(solutions)
         # for i in torch.arange(solutions.size(0)):
         #     k = 0
         #     for j in range(2): 
         #         pre = j
         #         while True:
         #             current_nodes = solutions[i,pre]
         #             k += 1
         #             visited_time[i,current_nodes] = k
         #             pre = solutions[i,pre]
         #             if pre == j:
         #                 break
         ...
         # return 
         return torch.gather(position_enc_new, 1, index), visited_time, top2 if clac_stacks else None

        
    def forward(self, x, solutions, clac_stacks = False, problem=None):
        pos_enc, visited_time, top2 = self.position_encoding(solutions, self.embedding_dim, clac_stacks,problem)
