...
class Actor(nn.Module):
    ...
    def forward(self, problem, x_in, solution, exchange, action_record, do_sample = False, fixed_action = None, require_entropy = False, to_critic = False, only_critic  = False):
        ...
        h_embed, h_pos, visited_time, top2 = self.embedder(x_in, solution, self.clac_stacks, problem)
        ...
        # _, visited_time = problem.getMcSeqBatch(solution)
        visited_order_map1 = problem.get_visited_order_map(visited_time)
        _, visited_time = problem.getMcSeqBatch(solution, reverse=True)
        visited_order_map2 = problem.get_visited_order_map(visited_time)
        visited_order_map = visited_order_map1 | visited_order_map2
